...
class LoanControllerFT extends PostgresContainerTestSpecification{
	...
	def "GET /api/loans/{loanId}/repay should call transferService with proper PaymentOrderEvent \
		...
		given:
			...
			def investmentSchedule = LoanTestHelper.createRepaymentSchedule(repaymentsNumber, totalLoanValue)
			def loanSchedule = LoanTestHelper.createRepaymentSchedule(repaymentsNumber, totalLoanValue)
			def investment = LoanTestHelper.createInvestmentWithSchedule(investmentSchedule)
			def loan = LoanTestHelper.crateActiveEmptyLoan(repaymentsNumber)
