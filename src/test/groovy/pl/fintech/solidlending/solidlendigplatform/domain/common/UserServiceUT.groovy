...
import pl.fintech.solidlending.solidlendigplatform.domain.payment.PaymentTestHelper
...
class UserServiceUT extends Specification {
	...
	def "findUser should return existing borrower from repository, given borrower name"(){
		given:
			...
			def borrower = PaymentTestHelper.createBorrower(UUID.randomUUID(), borrowerName)
		when:
	...
	def "findUser should return existing lender from repository, given lender name"(){
		given:
			...
			def lender = PaymentTestHelper.createLender(UUID.randomUUID(), lenderName)
		when:
